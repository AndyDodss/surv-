        ...
        if q.name ==  request.POST.get(str(q.id)) :
            if q.Answer_type =='rate' :
                ...
                #return HttpResponse(type(answer))
                    ...
                    rate = answer
                    comment = ''
                    #return HttpResponse(request.POST.get('waiter'))
...
def logout(request):
    ...
    del request.session['person_name']
    return render(request, 'home.html')
...
def login(request):
    if request.method == 'POST':
        ...
        try:
            ...
            request.session['person_role'] = Person.role
